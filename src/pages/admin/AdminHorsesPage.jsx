        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { label: "Registration Date", value: formatDate(horse.registeredAt || horse.createdAt) },
            ].map(({ label, value }) => (
          ...
          <div className="bg-surface-container-lowest rounded-xl p-4 border border-white/5">
            <p className="text-xs text-on-surface-variant uppercase tracking-wider mb-2">Horse Owner</p>
...
export default function AdminHorsesPage() {
  ...
  const loadHorses = useCallback(async () => {
    ...
    try {
      // Backend note:
      // - GET /api/horses: only returns { horseId, name, status, breed } - missing ownerName, color, birthYear
      // - GET /api/admin/horses/pending: returns full fields (ownerName, createdAt, etc.)
      //
      // Strategy:
      // 1. Get the full list from the pending API (has ownerName, color, birthYear)
      // 2. Get status from /api/horses to merge
      // 3. Horses in pending = status "Pending"
      // 4. Horses not in pending = use status from /api/horses
      ...
      // Build merged horses from pending list (has the most complete fields)
      const merged = [];
      if (pendingRes.status === "fulfilled") {
        ...
        (Array.isArray(data) ? data : []).forEach((h) => {
          merged.push({
            ...h,
            status: "Pending", // Override with status Pending
          });
        });
      }

      // Add horses from allHorses that are NOT in pending
      const pendingIds = new Set(merged.map((h) => h.horseId));
      if (allRes.status === "fulfilled") {
        const allHorses = Array.isArray(allRes.value.data) ? allRes.value.data : [];
        allHorses.forEach((h) => {
          if (!pendingIds.has(h.horseId)) {
            // Get status from map, keep fields from pending if available
  ...
  const handleRestoreHorse = async (horseId) => {
    ...
    setError("");
    try {
      // Restore horse from Rejected → Approved by calling approveHorse
      await approveHorse(horseId);
      setRevokingId(null);
      await loadHorses();
  ...
  const handleRevoke = async (horseId) => {
    ...
    try {
      ...
      if (cancelledEntries > 0) {
        showSuccess(`Horse revoked. ${cancelledEntries} pending entries were cancelled.`);
      } else {
  ...
  const handleExport = () => {
    setExporting(true);
    try {
      const headers = ["ID", "Name", "Breed", "Color", "Horse Owner", "Email", "Status", "Registration Date", "Reason"];
    ...
    <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-on-surface-variant text-sm">
                Approve, manage, and oversee horse racing registration applications.
              </p>
            </div>
          </div>
          <button
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by horse name, owner, email, breed..."
            value={searchQuery}
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-on-surface-variant text-sm">Loading horse list...</span>
          </div>
        </div>
      ) : filtered.length === 0 ? (
        <div className="gs-card p-16 text-center">
          <div className="w-16 h-16 rounded-full bg-surface-container-high mx-auto mb-4 flex items-center justify-center">
            {/* Bug #2 fix — use a neutral icon instead of CircleCheck */}
            <Inbox className="w-8 h-8 text-primary/60" />
          </div>
          <h3 className="font-serif text-xl font-bold text-on-surface mb-2">
            {searchQuery ? "No results found" : "No horses found"}
          </h3>
          <p className="text-on-surface-variant text-sm">
            {searchQuery
              ? `No results for "${searchQuery}"`
              : activeTab === "All"
              ? "No horses have been registered yet."
              : activeTab === "Pending"
              ? "All horses have been reviewed! No horses are pending."
                  ...
                  <tr key={horse.horseId}>
                    ...
                    <td>
                      <div className="flex items-center gap-1.5">
                        ...
                        {/* Pending → rejection form open (reason REQUIRED) */}
                        {horse.status === HORSE_STATUS.PENDING && rejectingId === horse.horseId ? (
                        ...
                        {/* Approved → Revoke (change to Rejected) */}
                        ...
                        {/* Approved → Revoke confirmation open */}
                        {horse.status === HORSE_STATUS.APPROVED && revokingId === horse.horseId ? (
                        ...
                        {horse.status === HORSE_STATUS.REJECTED && revokingId !== horse.horseId ? (
                          <button
                            ...
                            title="Restore horse to Approved status"
                          >
                            {actionId === horse.horseId ? (
                        ...
                        {horse.status === HORSE_STATUS.REJECTED && revokingId === horse.horseId ? (
                          <div className="flex flex-col gap-1.5 w-60">
                            ...
                            {horse.rejectionReason ? (
                              <p className="text-[11px] text-amber-400 leading-snug bg-amber-500/10 border border-amber-500/20 rounded px-2 py-1.5">
                                Previous rejection/revocation reason: {horse.rejectionReason}
                              </p>
                            ) : (
                              <p className="text-[11px] text-on-surface-variant/60 italic leading-snug">
                                (Previous rejection reason unknown — data not available.)
                              </p>
                            )}
                            <div className="flex gap-1.5">
                              <button
